...
ap.add_argument("-t", "--modelalready", type=int)
args = vars(ap.parse_args())

if args["modelalready"] == 1:
    ...
    with open("pythonfiles/models/model.json", "w") as json_file:
        json_file.write(model_json)
    # serialize weights to HDF5
    model.save_weights("pythonfiles/models/model.h5")
...
# else:
    ...
    loaded_model.compile(loss=losses.categorical_crossentropy,
                         optimizer=optimizer, metrics=["accuracy"])
    score = loaded_model.evaluate(testDataImgX, testDataLabelY, verbose=0)
    ...
    #          trainedNetwork.history["loss"], label="train_loss")
    # plt.plot(np.arange(0, N),
    #          trainedNetwork.history["val_loss"], label="val_loss")
    # plt.plot(np.arange(0, N), trainedNetwork.history["acc"], label="train_acc")
    # plt.plot(np.arange(0, N),
    #          trainedNetwork.history["val_acc"], label="val_acc")
